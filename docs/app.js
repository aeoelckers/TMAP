...
function buildQueryFromFilters() {
  const { typeFilter, regionFilter, communeFilter, minPrice, maxPrice, minArea, maxArea } = elements;
  const tokens = [];
...
function renderAutoSearches() {
  ...
  const { autoGrid, autoEmpty, autoHint } = elements;
  ...
  if (!query) {
    ...
    autoHint.textContent = 'Elige tipo, región o comuna para armar búsquedas en cada portal.';
    return;
  }
